...
class DataWriter:
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        for d in [self._color_dir, self._depth_dir, self._mask_dir, self._normals_dir, self._cam_pose_dir]:
    ...
    def write_data(self, data):
        ...
        for i in range(num_imgs):
            ...
            depth_img = os.path.join(self._depth_dir, "{:06d}.npy".format(self._file_idx))
            partial_depth_img = os.path.join(self._depth_dir, "partial_{:06d}.npy".format(self._file_idx))
            mask_img = os.path.join(self._mask_dir, "{:06d}.png".format(self._file_idx))
            normals_img = os.path.join(self._normals_dir, "{:06d}.npy".format(self._file_idx))
            cam_pose_file = os.path.join(self._cam_pose_dir, "{:06d}.npy".format(self._file_idx))
    ...
    def process_depth(self, depth, normals, segmask, cam_pose):
        ...
        processed_depth[np.where(segmask[:,:] == 1.)] = 0.
